## Running on Intel Core i5-6400 PC
...
* Download Intel MLC: https://www.intel.com/content/www/us/en/developer/articles/tool/intelr-memory-latency-checker.html.
* Move the donwloaded file to the `tools` folder
...
```
Then run `which fio` to find it's path
...
```
cd tools
git clone https://github.com/viktorleis/mmapbench.git
cd mmapbench
g++ -O3 -std=c++11 mmapbench.cpp -o mmapbench
```

**Running stream on one core and GPU task on another core**
```
sudo python3 -m mio c2m-p2m --ant gpu --ant_duration 120 --ant2 stream --ant2_num_cores 1 --ant2_cpus 0 --ant2_duration 120 --stats
```

**Running only the stream application**
```
sudo python3 -m mio c2m-iso \
  --ant stream \
  --ant_num_cores 1 \
  --ant_cpus 0 \
  --ant_duration 120 \
